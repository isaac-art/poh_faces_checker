    ...
    for image in images:
        if image.startswith('.'):
            ...
            if i >= start:
                ok, face_id, encoding = encode_face("faces", image)
                print(face_id)
                # pickle the encodind for this face to a file in encodings/face_id 
                if ok:
                    save_individual_encoding(face_id, encoding)
            else:
                print("skipping")
            i = i + 1
    return (encodings, face_ids)

def save_individual_encoding(face_id, encoding):
    # check encodings dir exists and make if not
    encodings_dir = os.path.join(os.path.abspath(os.getcwd()), "individual_encodings")
    if not os.path.isdir(encodings_dir):
        os.mkdir(encodings_dir)
    # save the encoding to a file
    with open(f"individual_encodings/{face_id}.pkl", 'wb') as f:
...
def main():
    # update_all() # uncomment to get graph data and profile images and store locally
